import React, { useContext } from 'react';
import { Link, useLocation, useNavigate } from "react-router-dom";
import AuthContext from '../../context/auth/authContext'; // Import AuthContext
import { FaUserCircle } from 'react-icons/fa'; // Import profile icon from react-icons (optional)

export const Navbar = () => {
  let location = useLocation();
  const authContext = useContext(AuthContext); // Access AuthContext
  const { token, logout } = authContext; // Destructure token and logout from context
  const navigate = useNavigate();

  const handleLogout = () => {
    logout();
    navigate('/login');
  };

  return (
    <>
      <nav className="navbar navbar-expand-lg navbar-light bg-light">
        <div className="container-fluid">
          <Link className="navbar-brand" to="/">iNotebook</Link>
          <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span className="navbar-toggler-icon"></span>
          </button>
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item">
                <Link className={`nav-link ${location.pathname === '/' ? 'active' : ''}`} aria-current="page" to="/">Home</Link>
              </li>
              <li className="nav-item">
                <Link className={`nav-link ${location.pathname === '/about' ? 'active' : ''}`} to="/about">About</Link>
              </li>
            </ul>
            <div className="d-flex align-items-center">
              {!token ? (
                <>
                  <Link className="btn btn-primary mx-2" to="/login" role="button">Login</Link>
                  <Link className="btn btn-primary mx-2" to="/signup" role="button">Signup</Link>
                </>
              ) : (
                <>
                  <Link className="btn btn-secondary mx-2" to="/profile" role="button">
                    <FaUserCircle size={24} /> {/* Profile icon */}
                  </Link>
                  <button className="btn btn-primary mx-2" onClick={handleLogout}>Logout</button>
                </>
              )}
            </div>
          </div>
        </div>
      </nav>
    </>
  );
};
